...
class SACClassifier(SAC):
    ...
    def _get_Q_target(self):
        ...
        self._observed_images = self._observations_ph[:, :self._env._env.env.image_shape]

        self._observation_logits = self._classifier([self._observations_ph])
        if self._reward_type == 'logits':
            self._reward_t = self._observation_logits
        elif self._reward_type == 'probabilities':
            self._reward_t = tf.nn.sigmoid(self._observation_logits)
        else:
    ...
    def _init_classifier_update(self):
        self._classifier_loss_t = tf.reduce_mean(
            ...
            logits=self._observation_logits, labels=self._label_ph))
